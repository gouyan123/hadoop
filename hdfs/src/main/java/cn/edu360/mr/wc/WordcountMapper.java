...
/** --maptask每从hdfs读取一行数据，调一次 map()方法--
 * 泛型解释如下：
 * KEYIN ：是maptask读取到的数据的key的类型，是一行的起始偏移量Long
 * VALUEIN:是maptask读取到的数据的value的类型，是一行的内容String
...
public class WordcountMapper extends Mapper<LongWritable, Text, Text, IntWritable>{
	@Override
	/*key：maptask读取到的行的偏移量；value：maptask读取到的行的内容；context：数据传送管道*/
