...
public class CollectTask extends TimerTask {
	...
	public void run() {
		...
		try {
			...
			// 构造一个log4j日志对象，将日志打到 log4j.appender.test1.File = d:/logs/collect/collect.log中，就是为了给目标路径下增加一些日志文件，以便上传到hdfs
			Logger logger = Logger.getLogger("logRollingFile");
			// 获取本次采集时的日期
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-HH");
			String day = sdf.format(new Date());
			File srcDir = new File(props.getProperty(Constants.LOG_SOURCE_DIR));
			// 列出日志源目录中需要采集的文件；new FilenameFilter() {}表示过滤；返回true表示将该文件返回到集合，返回false表示不返回这个文件；
			...
			for (File file : listFiles) {
				/*FileUtils为 apache.common通用工具类：将 file文件移动到 toUploadDir路径下，true表示如果toUploadDir路径不存在，则自动创建*/
